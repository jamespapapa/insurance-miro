...
echo "[dev] node=$(node -v 2>/dev/null || echo missing) npm=$(npm -v 2>/dev/null || echo missing)"
...
if port_open 127.0.0.1 5001 >/dev/null 2>&1; then
  echo "[dev] reusing existing backend on 5001"
else
...
if [ "${#pids[@]}" -eq 0 ]; then
  echo "[dev] backend/frontend already running"
