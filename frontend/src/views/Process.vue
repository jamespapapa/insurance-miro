      ...
      <div class="left-panel" :class="{ 'full-screen': isFullScreen }">
        <div class="panel-header">
          ...
          <div class="header-right">
            ...
            <div class="action-buttons">
                ...
                <button class="action-btn" @click="toggleFullScreen" :title="isFullScreen ? '전체화면 종료' : '전체화면 표시'">
        ...
        <div class="graph-container" ref="graphContainer">
          ...
          <div v-else-if="currentPhase < 1" class="graph-waiting">
            ...
            <p class="waiting-hint">생성이 완료되면 자동으로 그래프 구축이 시작됩니다</p>
          </div>
            ...
            <div class="phase-detail">
              <div class="detail-section">
                ...
                <div class="detail-content">
                  문서를 업로드한 후 LLM이 문서 내용을 분석하여, 여론 시뮬레이션에 적합한 온톨로지 구조(엔티티 유형 + 관계 유형)를 자동으로 생성합니다
                </div>
              </div>
              ...
              <div class="detail-section waiting-state" v-if="!projectData?.ontology && currentPhase === 0 && !ontologyProgress">
                <div class="waiting-hint">온톨로지 생성을 기다리는 중...</div>
              </div>
            </div>
          </div>
          ...
          <div class="process-phase" :class="{ 'active': currentPhase === 1, 'completed': currentPhase > 1 }">
            ...
            <div class="phase-detail">
              <div class="detail-section">
                <div class="detail-label">인터페이스 설명</div>
                <div class="detail-content">
                  생성된 온톨로지를 기반으로 문서를 블록으로 분할한 뒤 Zep API를 호출해 지식 그래프를 구축하고, 엔티티와 관계를 추출합니다
                </div>
              </div>
              
              <!-- 온톨로지 완료 대기 -->
              <div class="detail-section waiting-state" v-if="currentPhase < 1">
                <div class="waiting-hint">온톨로지 생성 완료를 기다리는 중...</div>
              </div>
          ...
          <div class="process-phase" :class="{ 'active': currentPhase === 2, 'completed': currentPhase > 2 }">
            <div class="phase-header">
              ...
              <div class="phase-info">
                ...
                <div class="phase-api">다음 단계로 진행할 준비</div>
              </div>
...
// 현재 프로젝트 ID('new'에서 실제 ID로 바뀔 수 있음)
...
const statusText = computed(() => {
  if (error.value) return '빌드 실패'
  if (currentPhase.value >= 2) return '빌드 완료'
...
const getPhaseStatusText = (phase) => {
  if (currentPhase.value > phase) return '완료됨'
...
const handleNewProject = async () => {
  ...
  if (!pending.isPending || pending.files.length === 0) {
    error.value = '업로드 대기 중인 파일이 없습니다. 홈으로 돌아가 다시 진행해 주세요'
    loading.value = false
    return
  }
  ...
  try {
    ...
    ontologyProgress.value = { message: '파일을 업로드하고 문서를 분석하는 중...' }
  ...
  try {
    ...
    if (response.success) {
      ...
      if (task.status === 'completed') {
        console.log('✅ 그래프 구축 완료, 전체 데이터를 로드하는 중...')
        ...
        buildProgress.value = {
          progress: 100,
          message: '구축 완료, 그래프를 로드하는 중...'
        }
        
        // 프로젝트 데이터를 다시 로드하여 graph_id 획득
        const projectResponse = await getProject(currentProjectId.value)
