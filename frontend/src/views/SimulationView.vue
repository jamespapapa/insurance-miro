      ...
      <div class="header-center">
        <div class="view-switcher">
          <button 
            ...
            @click="viewMode = mode"
          >
            {{ { graph: '그래프', split: '2단', workbench: '워크벤치' }[mode] }}
          </button>
        </div>
      </div>
...
/**
 * 실행 중인 시뮬레이션을 확인하고 종료
 * 사용자가 Step 3에서 Step 2로 돌아올 때 기본적으로 시뮬레이션을 종료하려는 것으로 간주
 */
const checkAndStopRunningSimulation = async () => {
  ...
  try {
    ...
    if (envStatusRes.success && envStatusRes.data?.env_alive) {
      addLog('시뮬레이션 환경이 실행 중임을 감지, 종료 중...')
      ...
      try {
        ...
        if (closeRes.success) {
          ...
          // 정상 종료에 실패하면 강제 중지 시도
          await forceStopSimulation()
        }
      } catch (closeErr) {
        addLog(`시뮬레이션 환경 종료 예외: ${closeErr.message}`)
        // 정상 종료 중 예외가 발생하면 강제 중지 시도
        await forceStopSimulation()
      }
    } else {
      // 환경은 실행 중이 아니지만 프로세스가 남아 있을 수 있으므로 시뮬레이션 상태 확인
      const simRes = await getSimulation(currentSimulationId.value)
      if (simRes.success && simRes.data?.status === 'running') {
        addLog('시뮬레이션 상태가 실행 중임을 감지, 중지 중...')
        await forceStopSimulation()
      }
    }
  } catch (err) {
    // 환경 상태 확인 실패는 이후 흐름에 영향을 주지 않음
...
const forceStopSimulation = async () => {
  try {
    ...
    addLog(`시뮬레이션 강제 중지 예외: ${err.message}`)
  }
}
...
onMounted(async () => {
  ...
  // 실행 중인 시뮬레이션을 확인하고 종료(사용자가 Step 3에서 돌아올 때)
