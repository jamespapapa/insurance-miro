          ...
          >
            ...
            <div class="timeline-card">
              ...
              <div class="card-body">
                ...
                <!-- 일반 폴백: 알 수 없는 타입이거나 content는 있으나 위에서 처리되지 않음 -->
...
// 시뮬레이션 경과 시간 포맷(라운드 수와 라운드당 분을 기준으로 계산)
...
const doStartSimulation = async () => {
  ...
  // 먼저 모든 상태를 초기화하여 이전 시뮬레이션의 영향이 없도록 보장
  resetAllState()
  
  isStarting.value = true
  startError.value = null
  addLog('양 플랫폼 병렬 시뮬레이션을 시작하는 중...')
  ...
  try {
    ...
    addLog('동적 그래프 업데이트 모드를 활성화했습니다')
    
    const res = await startSimulation(params)
    
    if (res.success && res.data) {
      if (res.data.force_restarted) {
        addLog('✓ 기존 시뮬레이션 로그를 정리하고 시뮬레이션을 다시 시작했습니다')
      }
...
// 각 플랫폼의 이전 라운드를 추적하여 변경을 감지하고 로그를 출력
...
const fetchRunStatus = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // 시뮬레이션이 완료되었는지 검사( runner_status 또는 플랫폼 완료 상태로 판단)
      const isCompleted = data.runner_status === 'completed' || data.runner_status === 'stopped'
      
      // 추가 확인: 백엔드가 아직 runner_status를 업데이트하지 못했지만 플랫폼이 이미 완료를 보고한 경우
      // twitter_completed 및 reddit_completed 상태를 검사하여 판단
...
const checkPlatformsCompleted = (data) => {
  ...
  // 최소 하나의 플랫폼이 완료되면, 활성화된 모든 플랫폼이 완료되었는지 확인
  // actions_count로 플랫폼 활성화 여부 판단 (count > 0 또는 running이 한때라도 true였던 경우)
  const twitterEnabled = (data.twitter_actions_count > 0) || data.twitter_running || twitterCompleted
  const redditEnabled = (data.reddit_actions_count > 0) || data.reddit_running || redditCompleted
  
  // 어떤 플랫폼도 활성화되지 않았다면 false 반환
...
const fetchRunStatusDetail = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      // all_actions를 사용해 전체 동작 목록 가져오기
      ...
      // 자동 스크롤하지 않고 사용자가 자유롭게 타임라인을 볼 수 있게 함
      // 새 동작은 하단에 추가됨
    }
  } catch (err) {
...
const handleNextStep = async () => {
  ...
  try {
    ...
    addLog(`✗ 보고서 생성 시작 중 예외 발생: ${err.message}`)
    isGeneratingReport.value = false
  }
}
