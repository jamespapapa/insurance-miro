            ...
            >
              ...
              <div class="timeline-content">
                ...
                <div class="timeline-body" :class="{ 'collapsed': isLogCollapsed(log) }" @click="toggleLogExpand(log)">
                  ...
                  <!-- Section Content Generated (콘텐츠 생성 완료, 하지만 전체 장은 아직 완료되지 않았을 수 있음) -->
...
const toggleRawResult = (timestamp, event) => {
  ...
  // DOM 업데이트를 기다린 후, 버튼이 같은 위치에 있도록 스크롤 위치를 조정
...
const toolConfig = {
  'insight_forge': {
    name: 'Deep Insight',
    color: 'purple',
    icon: 'lightbulb' // 전구 아이콘 - 통찰을 의미
  },
  'panorama_search': {
    name: 'Panorama Search',
    color: 'blue',
    icon: 'globe' // 지구 아이콘 - 파노라마 검색을 의미
  },
  'interview_agents': {
    name: 'Agent Interview',
    color: 'green',
    icon: 'users' // 사용자 아이콘 - 대화를 의미
  },
  'quick_search': {
    name: 'Quick Search',
    color: 'orange',
    icon: 'zap' // 번개 아이콘 - 빠름을 의미
  },
  'get_graph_statistics': {
    name: 'Graph Stats',
    color: 'cyan',
    icon: 'chart' // 차트 아이콘 - 통계를 의미
  },
  'get_entities_by_type': {
    name: 'Entity Query',
    color: 'pink',
    icon: 'database' // 데이터베이스 아이콘 - 엔티티를 의미
  }
}
...
const parseInterview = (text) => {
  ...
  try {
    ...
    // 인터뷰 인원 추출(예: "5 / 9 명의 시뮬레이션 Agent")
    ...
    const parseIndividualReasons = (reasonText) => {
      ...
      for (const line of lines) {
        ...
        // 형식2: - 선택이름（index X）：이유
        // 예: - 선택학부모_601（index 0）：학부모 집단 대표로서...
        if (!headerMatch) {
        ...
        } else if (currentName && line.trim() && !line.match(/^미선정|^종합하면|^최종 선택/)) {
          // 이유의 후속 줄(마지막 요약 단락 제외)
    ...
    interviewBlocks.forEach((block, index) => {
      ...
      if (answerMatch) {
        ...
        // 플랫폼 폴백 로직(구 포맷 호환: 플랫폼 표기가 하나만 있는 경우)
        if (!twitterMatch && redditMatch) {
          // Reddit 답변만 있는 경우, 자리표시 텍스트가 아닐 때만 기본 표시로 복사
          if (interview.redditAnswer && interview.redditAnswer !== '（해당 플랫폼에서 답변을 받지 못했습니다）') {
            interview.twitterAnswer = interview.redditAnswer
          }
        } else if (twitterMatch && !redditMatch) {
          if (interview.twitterAnswer && interview.twitterAnswer !== '（해당 플랫폼에서 답변을 받지 못했습니다）') {
            interview.redditAnswer = interview.twitterAnswer
          }
        } else if (!twitterMatch && !redditMatch) {
          // 플랫폼 구분 표기가 없음(아주 오래된 포맷), 전체를 답변으로 사용
          interview.twitterAnswer = answerText
        }
      }
      
      // 핵심 인용 추출(여러 따옴표 형식 호환)
      const quotesMatch = block.match(/\*\*핵심 인용:\*\*\n([\s\S]*?)(?=\n---|\n####|$)/)
      if (quotesMatch) {
        const quotesText = quotesMatch[1]
        // 우선 > "text" 형식 매칭
        let quoteMatches = quotesText.match(/> "([^"]+)"/g)
        // 폴백: > "text" 또는 > \u201Ctext\u201D(중국어 따옴표)
        if (!quoteMatches) {
...
const parseQuickSearch = (text) => {
  ...
  try {
    ...
    // 결과 개수 추출
    const countMatch = text.match(/총\s*(\d+)\s*건을 찾음/)
    if (countMatch) result.count = parseInt(countMatch[1])
    
    // 관련 사실 추출 - 전체 추출, 개수 제한 없음
    ...
    return () => h('div', { class: 'insight-display' }, [
      ...
      h('div', { class: 'insight-content' }, [
        ...
        activeTab.value === 'facts' && props.result.facts.length > 0 && h('div', { class: 'facts-panel' }, [
          ...
          }, expandedFacts.value ? `접기 ▲` : `전체 ${props.result.facts.length}개 펼치기 ▼`)
        ]),
        ...
        activeTab.value === 'relations' && props.result.relations.length > 0 && h('div', { class: 'relations-panel' }, [
          ...
          }, expandedRelations.value ? `접기 ▲` : `전체 ${props.result.relations.length}개 펼치기 ▼`)
        ]),
        
        // Sub-queries Tab
        activeTab.value === 'subqueries' && props.result.subQueries.length > 0 && h('div', { class: 'subqueries-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, '드리프트 쿼리 생성 분석 하위 질문'),
    ...
    return () => h('div', { class: 'panorama-display' }, [
      ...
      h('div', { class: 'panorama-content' }, [
        ...
        activeTab.value === 'active' && h('div', { class: 'facts-panel active-facts' }, [
          ...
          }, expandedActive.value ? `접기 ▲` : `전체 ${props.result.activeFacts.length}개 펼치기 ▼`)
        ]),
        ...
        activeTab.value === 'historical' && h('div', { class: 'facts-panel historical-facts' }, [
          ...
          }, expandedHistorical.value ? `접기 ▲` : `전체 ${props.result.historicalFacts.length}개 펼치기 ▼`)
        ]),
...
const InterviewDisplay = {
  ...
  setup(props) {
    ...
    const isPlaceholderText = (text) => {
      if (!text) return true
      const t = text.trim()
      return t === '（해당 플랫폼에서 응답을 받지 못했습니다）' || t === '(해당 플랫폼에서 응답을 받지 못했습니다)' || t === '[응답 없음]'
    }
    ...
    // 특정 질문에 대해 양 플랫폼 답변이 있는지 확인(자리표시 텍스트 필터)
    const hasMultiplePlatforms = (interview, qIdx) => {
      ...
      // 두 플랫폼 모두 실제 답변(자리표시 텍스트 아님)이 있고 내용이 다름
            ...
            return h('div', { class: 'qa-pair', key: qIdx }, [
              ...
              answerText && h('div', { class: ['qa-answer', { 'answer-placeholder': isPlaceholder }] }, [
                ...
                h('div', { class: 'qa-content' }, [
                  ...
                  // Expand/Collapse Button（자리표시 텍스트는 표시하지 않음）
      ...
      h('div', { class: ['quicksearch-content', { 'no-tabs': !showTabs.value }] }, [
        ...
        ((!showTabs.value) || activeTab.value === 'facts') && h('div', { class: 'facts-panel' }, [
          ...
          props.result.facts.length > INITIAL_SHOW_COUNT && h('button', {
            class: 'expand-btn',
            onClick: () => { expandedFacts.value = !expandedFacts.value }
          }, expandedFacts.value ? `접기 ▲` : `전체 펼치기 ${props.result.facts.length}개 ▼`)
        ]),
...
const renderMarkdown = (content) => {
  ...
  // 시작 부분의 2단계 제목(## xxx) 제거. 섹션 제목은 바깥에서 이미 표시됨
  ...
  // 줄바꿈 처리 - 빈 줄은 문단 구분, 단일 줄바꿈은 <br>로 변환
  html = html.replace(/\n\n/g, '</p><p class="md-p">')
  html = html.replace(/\n/g, '<br>')
  ...
  // <p><br> 다음에 블록 요소가 오는 경우 정리(불필요한 빈 줄로 인해)
  html = html.replace(/<p class="md-p">(<br>\s*)+(<ul|<ol|<blockquote|<pre|<hr)/g, '$2')
  // 연속된 <br> 태그 정리
  html = html.replace(/(<br>\s*){2,}/g, '<br>')
  // 블록 요소 뒤에 바로 오는 문단 시작 태그 앞의 <br> 정리
  html = html.replace(/(<\/ol>|<\/ul>|<\/blockquote>)<br>(<p|<div)/g, '$1$2')

  // 연속되지 않은 순서 목록 번호 수정: 단일 항목 <ol>이 문단 내용으로 끊길 때 번호가 증가하도록 유지
...
const getLogLevelClass = (log) => {
  ...
  // INFO 기본 색상을 사용하며 success로 표시하지 않음
  return ''
}
      ...
      if (newLogs.length > 0) {
        newLogs.forEach(log => {
          ...
          if (log.action === 'report_complete') {
            ...
            currentSectionIndex.value = null  // loading 상태가 반드시 초기화되도록 보장
        ...
        nextTick(() => {
          if (rightPanel.value) {
            // 작업이 완료되면 상단으로 스크롤, 아니면 최신 로그를 따라 하단으로 스크롤
            if (isComplete.value) {
...
// 최종 답변 내용 추출 - LLM response에서 섹션 내용을 추출
const extractFinalContent = (response) => {
  if (!response) return null
  
  // <final_answer> 태그 안의 내용을 추출 시도
  ...
  // Final Answer: 뒤의 내용 찾기 시도(여러 형식 지원)
  // 형식1: Final Answer:\n\n내용
  // 형식2: Final Answer: 내용
  ...
  // ## 또는 # 또는 >로 시작하면 직접적인 markdown 내용일 수 있음
  const trimmedResponse = response.trim()
  if (trimmedResponse.match(/^[#>]/)) {
    return trimmedResponse
  }
  
  // 내용이 길고 markdown 형식이 포함되어 있으면, 사고 과정을 제거한 뒤 반환 시도
...
/* Console Logs - Step3Simulation.vue 와 일관되게 유지 */
.console-logs {
  background: #000;
  color: #DDD;
  padding: 16px;
