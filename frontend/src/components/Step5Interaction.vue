          ...
          <div class="action-bar-tabs">
            ...
            <div class="agent-dropdown" v-if="profiles.length > 0">
              ...
              >
                ...
                <span>{{ selectedAgent ? selectedAgent.username : '세계 속 임의의 개체와 대화' }}</span>
              ...
              <div v-if="showAgentDropdown" class="dropdown-menu">
                ...
                >
                  ...
                  <div class="agent-info">
                    ...
                    <span class="agent-role">{{ agent.profession || '알 수 없는 직업' }}</span>
                  </div>
                </div>
              </div>
            </div>
        ...
        <div v-if="activeTab === 'chat'" class="chat-container">
          ...
          <div v-if="chatTarget === 'report_agent'" class="report-agent-tools-card">
            <div class="tools-card-header">
              ...
              <div class="tools-card-info">
                ...
                <div class="tools-card-subtitle">보고서 생성 에이전트의 빠른 대화 버전으로, 4가지 전문 도구를 호출할 수 있으며 MiroFish의 완전한 기억을 보유합니다</div>
              </div>
              ...
              <div class="tools-grid">
                <div class="tool-item tool-purple">
                  ...
                  <div class="tool-content">
                    ...
                    <div class="tool-desc">현실 세계의 시드 데이터와 시뮬레이션 환경 상태를 정렬하고 Global/Local Memory 메커니즘을 결합해 시공간을 넘나드는 심층 귀인 분석을 제공합니다</div>
                  </div>
                </div>
                <div class="tool-item tool-blue">
                  ...
                  <div class="tool-content">
                    ...
                    <div class="tool-desc">그래프 구조의 너비 우선 탐색 알고리즘을 기반으로 사건 전파 경로를 재구성하고 전체 정보 흐름의 토폴로지 구조를 포착합니다</div>
                  </div>
                </div>
                <div class="tool-item tool-orange">
                  ...
                  <div class="tool-content">
                    ...
                    <div class="tool-desc">GraphRAG 기반의 즉시 질의 인터페이스로 인덱싱 효율을 최적화하여 구체적인 노드 속성과 이산적 사실을 빠르게 추출합니다</div>
                  </div>
                </div>
                <div class="tool-item tool-green">
                  ...
                  <div class="tool-content">
                    ...
                    <div class="tool-desc">자율형 인터뷰로, 시뮬레이션 세계의 개체들과 병렬로 여러 차례 대화를 진행하며 비정형 관점 데이터와 심리 상태를 수집합니다</div>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div class="profile-card-header">
              ...
              <div class="profile-card-info">
                ...
                <div class="profile-card-meta">
                  ...
                  <span class="profile-card-profession">{{ selectedAgent.profession || '알 수 없는 직업' }}</span>
                </div>
              </div>
          ...
          <div class="chat-messages" ref="chatMessages">
            <div v-if="chatHistory.length === 0" class="chat-empty">
              ...
              <p class="empty-text">
                {{ chatTarget === 'report_agent' ? 'Report Agent와 대화하며 보고서 내용을 깊이 있게 알아보세요' : '시뮬레이션 개체와 대화하며 그들의 관점을 알아보세요' }}
              </p>
            </div>
            <div 
        ...
        <div v-if="activeTab === 'survey'" class="survey-container">
          ...
          <div class="survey-setup">
            <div class="setup-section">
              ...
              <div class="agents-grid">
                ...
                >
                  ...
                  <div class="checkbox-info">
                    ...
                    <span class="checkbox-role">{{ agent.profession || '알 수 없는 직업' }}</span>
                  </div>
              ...
              <div class="selection-actions">
                <button class="action-link" @click="selectAllAgents">모두 선택</button>
            ...
            <div class="setup-section">
              <div class="section-header">
                <span class="section-title">설문 문항</span>
              </div>
              <textarea 
                v-model="surveyQuestion"
                class="survey-input"
                placeholder="선택된 모든 대상에게 물어보고 싶은 질문을 입력하세요..."
                rows="3"
              ></textarea>
            </div>
          ...
          <div v-if="surveyResults.length > 0" class="survey-results">
            <div class="results-header">
              ...
              <span class="results-count">{{ surveyResults.length }} 개의 답변</span>
            </div>
              ...
              >
                <div class="result-header">
                  ...
                  <div class="result-info">
                    ...
                    <span class="result-role">{{ result.profession || '알 수 없는 직업' }}</span>
                  </div>
                </div>
...
const renderMarkdown = (content) => {
  ...
  // <p><br> 다음에 블록 레벨 요소가 오는 경우 정리(불필요한 빈 줄로 인한)
  html = html.replace(/<p class="md-p">(<br>\s*)+(<ul|<ol|<blockquote|<pre|<hr)/g, '$2')
  // 연속된 <br> 태그 정리
  html = html.replace(/(<br>\s*){2,}/g, '<br>')
  // 블록 레벨 요소 뒤에 이어지는 단락 시작 태그 앞의 <br> 정리
  html = html.replace(/(<\/ol>|<\/ul>|<\/blockquote>)<br>(<p|<div)/g, '$1$2')

  // 비연속 순서 리스트의 번호 수정: 단일 항목 <ol>이 단락 내용으로 끊길 때 번호 증가를 유지
...
const sendToAgent = async (message) => {
  ...
  if (chatHistory.value.length > 1) {
    const historyContext = chatHistory.value
      ...
      .slice(-6)
      .map(msg => `${msg.role === 'user' ? '질문자' : '너'}：${msg.content}`)
      .join('\n')
    prompt = `다음은 우리가 이전에 나눈 대화입니다：\n${historyContext}\n\n지금 나의 새로운 질문은：${message}`
  }
  ...
  if (res.success && res.data) {
    // 올바른 데이터 경로: res.data.result.results 는 객체 딕셔너리
    // 형식: {"twitter_0": {...}, "reddit_0": {...}} 또는 단일 플랫폼 {"reddit_0": {...}}
    const resultData = res.data.result || res.data
    const resultsDict = resultData.results || resultData
    
    // 객체 딕셔너리를 배열로 변환하고, 우선 reddit 플랫폼의 답변을 가져옴
    let responseContent = null
    const agentId = selectedAgentIndex.value
    
    if (typeof resultsDict === 'object' && !Array.isArray(resultsDict)) {
      // 우선 reddit 플랫폼 답변, 다음 twitter
    ...
    if (responseContent) {
      ...
      addLog(`${selectedAgent.value.username}가 응답했습니다`)
    } else {
...
const submitSurvey = async () => {
  ...
  addLog(`설문을 ${selectedAgents.value.size}명의 대상에게 전송...`)
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      addLog(`${surveyResults.value.length}개의 응답을 받았습니다`)
    } else {
      throw new Error(res.error || '요청 실패')
    }
  } catch (err) {
...
const loadProfiles = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      addLog(`시뮬레이션 개체 ${profiles.value.length}개를 로드했습니다`)
    }
  } catch (err) {
...
/* 순서 있는 목록 번호 수정 - CSS 카운터를 사용해 여러 ol이 연속 번호를 가지도록 함 */
.message-text {
