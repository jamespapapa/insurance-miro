    ...
    <div class="graph-container" ref="graphContainer">
      ...
      <div v-if="graphData" class="graph-view">
        ...
        <div v-if="showSimulationFinishedHint" class="graph-building-hint finished-hint">
          ...
          <span class="hint-text">아직 소량의 내용이 처리 중이니, 잠시 후 그래프를 수동으로 새로고침하는 것을 권장합니다</span>
      ...
      <div v-else class="graph-state">
        ...
        <p class="empty-text">본체 생성 대기 중...</p>
      </div>
    </div>
...
const showSimulationFinishedHint = ref(false) // 시뮬레이션 종료 후 힌트
...
watch(() => props.isSimulating, (newValue, oldValue) => {
  if (wasSimulating.value && !newValue) {
    // 시뮬레이션 상태에서 비시뮬레이션 상태로 전환되면 종료 힌트 표시
    showSimulationFinishedHint.value = true
  }
  wasSimulating.value = newValue
}, { immediate: true })

// 자기 루프 항목 펼치기/접기 상태 전환
...
const renderGraph = () => {
  ...
  // edge data 처리, 동일한 노드 쌍 사이의 edge 개수와 인덱스 계산
  const edgePairCount = {}
  const selfLoopEdges = {} // 노드별로 그룹화된 self-loop edge
  const tempEdges = edgesData
    .filter(e => nodeIds.has(e.source_node_uuid) && nodeIds.has(e.target_node_uuid))
  
  // 각 노드 쌍 사이의 edge 개수를 집계하고, self-loop edge를 수집
  tempEdges.forEach(e => {
  ...
  // 현재 각 노드 쌍을 처리하는 중 몇 번째 edge인지 기록
  const edgePairIndex = {}
  const processedSelfLoopNodes = new Set() // 처리된 self-loop 노드
  ...
  tempEdges.forEach(e => {
    ...
    if (isSelfLoop) {
      // self-loop edge - 각 노드는 병합된 self-loop를 1개만 추가
      ...
      edges.push({
        ...
        rawData: {
          ...
          selfLoopEdges: allSelfLoops // 모든 self-loop edge의 상세 정보 저장
        }
      })
      return
    }
    ...
    // edge 방향이 표준화된 방향과 일치하는지 판단 (source UUID < target UUID)
    const isReversed = e.source_node_uuid > e.target_node_uuid
    
    // 곡률 계산: 여러 edge면 분산, 단일 edge면 직선
    let curvature = 0
    if (totalCount > 1) {
      // 곡률을 균일하게 분포시켜 확실히 구분
      // 곡률 범위는 edge 수에 따라 증가; edge가 많을수록 곡률 범위가 커짐
      const curvatureRange = Math.min(1.2, 0.6 + totalCount * 0.15)
      curvature = ((currentIndex / (totalCount - 1)) - 0.5) * curvatureRange * 2
      
      // edge 방향이 표준화 방향과 반대면 곡률을 반전
      // 이렇게 하면 모든 edge가 같은 기준 좌표계에서 분포되어, 방향 차이로 겹치지 않음
      if (isReversed) {
        curvature = -curvature
      }
    }
  ...
  // Simulation - edge 개수에 따라 노드 간격을 동적으로 조정
  const simulation = d3.forceSimulation(nodes)
    .force('link', d3.forceLink(edges).id(d => d.id).distance(d => {
      // 이 노드 쌍 사이의 edge 개수에 따라 거리를 동적으로 조정
      // 기본 거리 150, edge가 하나 늘 때마다 40 증가
    ...
    // 중심을 향한 인력을 추가하여, 독립된 노드 그룹이 중앙 영역으로 모이게 함
  ...
  const getLinkPath = (d) => {
    ...
    // self-loop 감지
    if (d.isSelfLoop) {
      // self-loop: 노드에서 출발해 다시 돌아오는 원호를 그리기
      const loopRadius = 30
      // 노드 오른쪽에서 출발해 한 바퀴 돌아오기
      const x1 = sx + 8  // 시작점 오프셋
      const y1 = sy - 4
      const x2 = sx + 8  // 끝점 오프셋
      const y2 = sy + 4
      // 원호로 self-loop 그리기 (sweep-flag=1 시계 방향)
    ...
    // 곡선 제어점 계산 - edge 개수와 거리에 따라 동적으로 조정
    const dx = tx - sx, dy = ty - sy
    const dist = Math.sqrt(dx * dx + dy * dy)
    // 연결선 방향에 수직인 오프셋을 거리 비율로 계산해 곡선이 확실히 보이게 함
    // edge가 많을수록 오프셋이 거리에서 차지하는 비율이 커짐
    const pairTotal = d.pairTotal || 1
    const offsetRatio = 0.25 + pairTotal * 0.05 // 기본 25%, edge가 하나 늘 때마다 5% 증가
  ...
  const getLinkMidpoint = (d) => {
    ...
    // 2차 베지에 곡선 공식 B(t) = (1-t)²P0 + 2(1-t)tP1 + t²P2, t=0.5
  ...
  // Link labels background (흰색 배경이 글자를 더 선명하게 함)
  ...
  const node = nodeGroup.selectAll('circle')
    ...
    .call(d3.drag()
      .on('start', (event, d) => {
        // 위치만 기록하고 시뮬레이션은 재시작하지 않음(클릭과 드래그 구분)
        d.fx = d.x
        d.fy = d.y
      ...
      .on('drag', (event, d) => {
        // 실제로 드래그가 시작되었는지 감지(이동이 임계값 초과)
        const dx = event.x - d._dragStartX
        const dy = event.y - d._dragStartY
        const distance = Math.sqrt(dx * dx + dy * dy)
        
        if (!d._isDragging && distance > 3) {
          // 처음으로 실제 드래그를 감지했을 때만 시뮬레이션 재시작
          d._isDragging = true
  ...
  simulation.on('tick', () => {
    ...
    // 간선 라벨 위치 업데이트(회전 없음, 수평 표시가 더 선명함)
