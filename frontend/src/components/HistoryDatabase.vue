<template>
  ...
  >
    ...
    <div v-if="projects.length > 0" class="cards-container" :class="{ expanded: isExpanded }" :style="containerStyle">
      ...
      >
        <!-- 카드 헤더: simulation_id와 기능 사용 가능 상태 -->
        ...
        <div class="card-files-wrapper">
          ...
          <div class="files-empty" v-else>
            ...
            <span class="empty-file-text">파일이 없습니다</span>
          </div>
        </div>
          ...
          <div class="modal-content">
            ...
            <div class="modal-body">
              ...
              <div class="modal-section">
                ...
                <div class="modal-empty" v-else>현재 연결된 파일이 없습니다</div>
              </div>
            </div>
...
const containerStyle = computed(() => {
  ...
  // 실제 필요한 높이 계산: 행 수 * 카드 높이 + (행 수-1) * 간격 + 약간의 하단 여백
...
const getCardStyle = (index) => {
  ...
  if (isExpanded.value) {
    ...
    // 현재 행의 카드 개수를 계산하여 각 행이 가운데 정렬되도록 보장
    ...
    const x = offset * 35
    // 시작 위치를 조정하고, 제목에 가깝게 하되 적절한 간격을 유지
...
// simulation_id 표시 형식화（앞 6자리만 잘라냄）
  ...
  observer = new IntersectionObserver(
    (entries) => {
      entries.forEach((entry) => {
        ...
        // 실행 대기 중인 목표 상태 업데이트(애니메이션 중이든 아니든 최신 목표 상태를 기록해야 함)
        pendingState = shouldExpand
        
        // 이전 디바운스 타이머 제거(새로운 스크롤 의도가 기존 것을 덮어씀)
        if (expandDebounceTimer) {
          clearTimeout(expandDebounceTimer)
          expandDebounceTimer = null
        }
        
        // 애니메이션 중이면 상태만 기록하고, 애니메이션이 끝난 뒤 처리
        if (isAnimating) return
        
        // 목표 상태가 현재 상태와 같으면 처리할 필요 없음
        if (shouldExpand === isExpanded.value) {
          pendingState = null
          return
        }
        
        // 디바운스를 사용해 상태 전환을 지연하여 빠른 깜빡임을 방지
        // 펼칠 때는 지연을 짧게(50ms), 접을 때는 지연을 길게(200ms) 하여 안정성을 높임
        ...
        expandDebounceTimer = setTimeout(() => {
          ...
          // 실행 대기 상태가 여전히 실행이 필요한지 확인(후속 스크롤에 의해 덮였을 수 있음)
          ...
          // 애니메이션 완료 후 잠금을 해제하고, 처리 대기 중인 상태 변경이 있는지 확인
          setTimeout(() => {
            isAnimating = false
            
            // 애니메이션 종료 후, 새로운 실행 대기 상태가 있는지 확인
            if (pendingState !== null && pendingState !== isExpanded.value) {
              // 너무 빠른 전환을 피하기 위해, 잠시 지연한 뒤 실행
              expandDebounceTimer = setTimeout(() => {
                if (pendingState !== null && pendingState !== isExpanded.value) {
                  isAnimating = true
                  isExpanded.value = pendingState
    ...
    {
      // 여러 임계값을 사용해 감지를 더 부드럽게 함
      threshold: [0.4, 0.6, 0.8],
      // rootMargin을 조정해 뷰포트 하단을 위로 줄이고, 더 많이 스크롤해야 펼치기가 트리거되도록 함
...
// 라우트 변경을 감시하고, 홈으로 돌아오면 데이터를 다시 로드
...
onMounted(async () => {
  // DOM 렌더링이 완료된 후에 데이터를 로드하도록 보장
  await nextTick()
  await loadHistory()
  
  // DOM 렌더링을 기다린 뒤 관찰자 초기화
  setTimeout(() => {
    initObserver()
  }, 100)
})

// keep-alive를 사용하면，컴포넌트가 활성화될 때 데이터를 다시 로드
onActivated(() => {
  loadHistory()
})
...
/* CSS 배경 패턴을 사용해 고정 간격의 정사각형 그리드 생성 */
.grid-pattern {
  position: absolute;
  top: 0;
  left: 0;
  ...
  /* 왼쪽 위 모서리에서부터 위치를 잡고, 높이가 변할 때는 아래쪽으로만 확장하여 기존 그리드 위치에 영향을 주지 않음 */
...
.cards-container {
  ...
  /* min-height는 JS로 동적으로 계산되며, 카드 수에 따라 자동으로 적응 */
}
