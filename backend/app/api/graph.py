"""
그래프 관련 API 라우트
프로젝트 컨텍스트 메커니즘을 채택하여, 서버 측에서 상태를 영속화
"""
...
# ============== 인터페이스 1: 파일 업로드 및 본체 생성 ==============

@graph_bp.route('/ontology/generate', methods=['POST'])
def generate_ontology():
    """
    인터페이스 1: 파일을 업로드하고 분석하여 본체 정의 생성
    
    요청 방식: multipart/form-data
    
    파라미터:
        files: 업로드한 파일(PDF/MD/TXT), 여러 개 가능
    ...
    try:
        ...
        if not document_texts:
            ...
            return jsonify({
                "success": False,
                "error": "어떤 문서도 성공적으로 처리하지 못했습니다. 파일 형식을 확인해 주세요"
            }), 400
        ...
        logger.info(f"본체 생성 완료: {entity_count}개 엔티티 유형, {edge_count}개 관계 유형")
        ...
        project.status = ProjectStatus.ONTOLOGY_GENERATED
        ProjectManager.save_project(project)
        logger.info(f"=== 본체 생성 완료 === 프로젝트 ID: {project.project_id}")
...
def build_graph():
    """
    인터페이스 2: project_id에 따라 그래프를 구축
    ...
    try:
        ...
        # 프로젝트 상태 확인
        force = data.get('force', False)  # 강제로 다시 구축
        
        if project.status == ProjectStatus.CREATED:
            return jsonify({
                "success": False,
                "error": "프로젝트의 온톨로지가 아직 생성되지 않았습니다. 먼저 /ontology/generate 를 호출해 주세요"
            }), 400
        
        if project.status == ProjectStatus.GRAPH_BUILDING and not force:
            return jsonify({
                "success": False,
                "error": "그래프가 구축 중입니다. 중복 제출하지 마세요. 강제 재구축이 필요하면 force: true 를 추가해 주세요",
        ...
        def build_task():
            ...
            try:
                ...
                task_manager.update_task(
                    task_id,
                    message=f"{total_chunks}개의 텍스트 블록 추가 시작...",
                    progress=15
                )
                ...
                # Zep 처리가 완료될 때까지 대기(각 episode의 processed 상태 조회)
                task_manager.update_task(
                    task_id,
                    message="Zep가 데이터를 처리할 때까지 대기...",
                    progress=55
                )
        ...
        return jsonify({
            ...
            "data": {
                "project_id": project_id,
                "task_id": task_id,
                "message": "그래프 구축 작업이 시작되었습니다. /task/{task_id}를 통해 진행 상황을 조회하세요"
            }
        })
