"""
Report API 라우팅
모의 보고서 생성, 조회, 대화 등의 인터페이스를 제공합니다
"""
...
def generate_report():
    ...
    이는 시간이 오래 걸리는 작업이며, 인터페이스는 즉시 task_id를 반환합니다,
    GET /api/report/generate/status 를 사용해 진행 상황을 조회합니다
    
    요청(JSON):
        {
            "simulation_id": "sim_xxxx",    // 필수, 모의 ID
            "force_regenerate": false        // 선택, 강제로 다시 생성
        }
    ...
    try:
        ...
        if not graph_id:
            return jsonify({
                "success": False,
                "error": "그래프 ID가 누락되었습니다. 그래프가 이미 구축되었는지 확인해 주세요"
            }), 400
        ...
        if not simulation_requirement:
            return jsonify({
                "success": False,
                "error": "시뮬레이션 요청 설명이 누락되었습니다"
            }), 400
        
        # report_id를 미리 생성하여, 즉시 프론트엔드에 반환할 수 있도록 함
        import uuid
        ...
        return jsonify({
            ...
            "data": {
                ...
                "task_id": task_id,
                "status": "generating",
                "message": "보고서 생성 작업이 시작되었습니다, /api/report/generate/status 를 통해 진행 상황을 조회해 주세요",
...
def get_generate_status():
    ...
    try:
        ...
        # simulation_id가 제공되면, 먼저 이미 완료된 보고서가 있는지 확인
        if simulation_id:
            existing_report = ReportManager.get_report_by_simulation(simulation_id)
            if existing_report and existing_report.status == ReportStatus.COMPLETED:
                return jsonify({
...
def get_report_by_simulation(simulation_id: str):
    ...
    try:
        ...
        if not report:
            return jsonify({
                "success": False,
                "error": f"해당 시뮬레이션에는 아직 보고서가 없습니다: {simulation_id}",
                "has_report": False
            }), 404
...
def chat_with_report_agent():
    ...
    Report Agent는 대화 중에 스스로 검색 도구를 호출해 질문에 답할 수 있습니다
...
def get_report_progress(report_id: str):
    ...
    반환：
        {
            ...
            "data": {
                "status": "generating",
                "progress": 45,
                "message": "섹션을 생성 중: 핵심 발견",
                "current_section": "핵심 발견",
                "completed_sections": ["경영진 요약", "시뮬레이션 배경"],
    ...
    try:
        ...
        if not progress:
            return jsonify({
                "success": False,
                "error": f"보고서가 존재하지 않거나 진행 정보가 제공되지 않습니다: {report_id}"
            }), 404
...
def get_report_sections(report_id: str):
    """
    이미 생성된 장 목록을 가져옵니다(장별 출력)
    
    프론트엔드는 전체 보고서가 완료될 때까지 기다릴 필요 없이 이 인터페이스를 폴링하여 이미 생성된 장 내용을 가져올 수 있습니다
...
def get_agent_log(report_id: str):
    """
    Report Agent의 상세 실행 로그를 가져옵니다
    
    보고서 생성 과정의 각 단계를 실시간으로 가져오며, 다음을 포함합니다:
    - 보고서 시작, 계획 시작/완료
    - 각 섹션의 시작, 도구 호출, LLM 응답, 완료
    - 보고서 완료 또는 실패
    
    Query 파라미터:
        from_line: 몇 번째 줄부터 읽기 시작할지(선택, 기본값 0, 증분 가져오기용)
...
def stream_agent_log(report_id: str):
    """
    전체 Agent 로그를 가져옵니다（한 번에 전부 가져오기）
...
def get_console_log(report_id: str):
    """
    Report Agent의 콘솔 출력 로그를 가져옵니다
    
    보고서 생성 과정의 콘솔 출력을 실시간으로 가져옵니다(INFO, WARNING 등).
    이는 agent-log 인터페이스가 반환하는 구조화된 JSON 로그와 다르며,
    순수 텍스트 형식의 콘솔 스타일 로그입니다.
    
    Query 파라미터：
        from_line: 몇 번째 줄부터 읽기 시작할지(선택, 기본값 0, 증분 조회용)
    ...
    반환：
        {
            ...
            "data": {
                "logs": [
                    "[19:46:14] INFO: 검색 완료: 관련 사실 15건을 찾았습니다",
