...
2. 시뮬레이션 스크립트가 명령 디렉터리를 폴링하여 명령을 실행하고 responses/ 디렉터리에 응답을 기록
3. Flask가 응답 디렉터리를 폴링하여 결과를 가져옴
"""
...
class SimulationIPCClient:
    ...
    시뮬레이션 프로세스에 명령을 전송하고 응답을 기다리는 데 사용
    """
    ...
    def send_command(
        ...
        Raises:
            TimeoutError: 응답을 기다리는 시간이 초과되었습니다
        """
    ...
    def send_interview(
        ...
        Args:
            ...
            platform: 지정 플랫폼(선택 사항)
                ...
                - None: 양 플랫폼 시뮬레이션에서는 두 플랫폼을 동시에 인터뷰하고, 단일 플랫폼 시뮬레이션에서는 해당 플랫폼을 인터뷰
            timeout: 타임아웃 시간
            
        Returns:
            IPCResponse, result 필드에 인터뷰 결과가 포함됨
        """
        args = {
            "agent_id": agent_id,
            "prompt": prompt
    ...
    def send_batch_interview(
        ...
        Args:
            ...
            platform: 기본 플랫폼（선택，각 인터뷰 항목의platform으로 덮어씀）
        ...
        Returns:
            IPCResponse，result필드에 모든 인터뷰 결과가 포함됨
        """
...
class SimulationIPCServer:
    """
    IPC 서버 시뮬레이션(시뮬레이션 스크립트 측에서 사용)
    
    명령 디렉터리를 폴링하여 명령을 실행하고 응답을 반환
    """
    ...
    def poll_commands(self) -> Optional[IPCCommand]:
        """
        명령 디렉터리를 폴링하여 처리 대기 중인 첫 번째 명령을 반환합니다
