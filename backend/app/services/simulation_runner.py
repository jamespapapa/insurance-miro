...
class SimulationRunState:
    ...
    # 각 플랫폼별 독립 라운드 및 시뮬레이션 시간(두 플랫폼 병렬 표시용)
    ...
    # 플랫폼 완료 상태(actions.jsonl에서 simulation_end 이벤트를 감지하여)
...
class SimulationRunner:
    ...
    _stdout_files: Dict[str, Any] = {}  # stdout 파일 핸들을 저장
    _stderr_files: Dict[str, Any] = {}  # stderr 파일 핸들을 저장
    ...
    def start_simulation(
        ...
        max_rounds: int = None,  # 최대 시뮬레이션 라운드 수(선택, 너무 긴 시뮬레이션을 자르기 위한 용도)
        ...
        Args:
            ...
            max_rounds: 최대 시뮬레이션 라운드 수(선택, 너무 긴 시뮬레이션을 자르기 위한 용도)
        ...
        if max_rounds is not None and max_rounds > 0:
            ...
            if total_rounds < original_rounds:
                logger.info(f"라운드 수가 잘렸습니다: {original_rounds} -> {total_rounds} (max_rounds={max_rounds})")
        ...
        # 그래프 메모리 업데이트가 활성화되어 있으면, 업데이터 생성
        if enable_graph_memory_update:
            if not graph_id:
                raise ValueError("그래프 메모리 업데이트를 활성화할 때는 graph_id를 반드시 제공해야 합니다")
        ...
        # 어떤 스크립트를 실행할지 결정(스크립트는 backend/scripts/ 디렉터리에 위치)
        if platform == "twitter":
        ...
        try:
            ...
            # 최대 라운드 수가 지정된 경우, 명령행 인자에 추가
            if max_rounds is not None and max_rounds > 0:
                cmd.extend(["--max-rounds", str(max_rounds)])
            
            # stdout/stderr 파이프 버퍼가 가득 차서 프로세스가 막히는 것을 피하기 위해 메인 로그 파일 생성
            main_log_path = os.path.join(sim_dir, "simulation.log")
            main_log_file = open(main_log_path, 'w', encoding='utf-8')
            
            # 하위 프로세스 환경 변수를 설정하여 Windows에서 UTF-8 인코딩을 사용하도록 보장
            # 이는 서드파티 라이브러리(예: OASIS)가 파일을 읽을 때 인코딩을 지정하지 않는 문제를 수정할 수 있음
            env = os.environ.copy()
            env['PYTHONUTF8'] = '1'  # Python 3.7+ 지원, 모든 open()이 기본으로 UTF-8을 사용하도록 함
            env['PYTHONIOENCODING'] = 'utf-8'  # stdout/stderr가 UTF-8을 사용하도록 보장
            
            # 작업 디렉터리를 시뮬레이션 디렉터리로 설정(데이터베이스 등 파일이 여기 생성됨)
            # start_new_session=True로 새 프로세스 그룹을 만들어 os.killpg로 모든 하위 프로세스를 종료할 수 있도록 보장
            process = subprocess.Popen(
                cmd,
                cwd=sim_dir,
                stdout=main_log_file,
                stderr=subprocess.STDOUT,  # stderr 도 같은 파일에 기록
                text=True,
                encoding='utf-8',  # 명시적으로 인코딩 지정
                bufsize=1,
                env=env,  # UTF-8 설정이 포함된 환경 변수를 전달
                start_new_session=True,  # 새 프로세스 그룹을 생성해 서버 종료 시 관련된 모든 프로세스를 종료할 수 있도록 함
            )
            
            # 파일 핸들을 저장해 이후에 닫을 수 있도록 함
            cls._stdout_files[simulation_id] = main_log_file
            cls._stderr_files[simulation_id] = None  # 더 이상 별도의 stderr가 필요 없음
        ...
        try:
                ...
                try:
                    if os.path.exists(main_log_path):
                        with open(main_log_path, 'r', encoding='utf-8') as f:
                            error_info = f.read()[-2000:]  # 마지막 2000자만 취함
                except Exception:
                    pass
            ...
            if cls._graph_memory_enabled.get(simulation_id, False):
                try:
                    ...
                    logger.info(f"그래프 메모리 업데이트를 중지함: simulation_id={simulation_id}")
                except Exception as e:
                    ...
                    if line:
                        try:
                            ...
                            if "event_type" in action_data:
                                ...
                                # simulation_end 이벤트를 감지해 플랫폼 완료로 표시
                                if event_type == "simulation_end":
                                    if platform == "twitter":
                                        state.twitter_completed = True
                                        state.twitter_running = False
                                        logger.info(f"Twitter 시뮬레이션이 완료됨: {state.simulation_id}, total_rounds={action_data.get('total_rounds')}, total_actions={action_data.get('total_actions')}")
                                    elif platform == "reddit":
                                        state.reddit_completed = True
                                        state.reddit_running = False
                                        logger.info(f"Reddit 시뮬레이션이 완료됨: {state.simulation_id}, total_rounds={action_data.get('total_rounds')}, total_actions={action_data.get('total_actions')}")
                                    
                                    # 활성화된 모든 플랫폼이 완료되었는지 확인
                                    # 하나의 플랫폼만 실행했다면 그 플랫폼만 확인
                                    # 두 플랫폼을 실행했다면 둘 다 완료되어야 함
                                    all_completed = cls._check_all_platforms_completed(state)
                                    if all_completed:
                                        state.runner_status = RunnerStatus.COMPLETED
                                        state.completed_at = datetime.now().isoformat()
                                        logger.info(f"모든 플랫폼 시뮬레이션이 완료됨: {state.simulation_id}")
                                ...
                                # 라운드 정보 업데이트( round_end 이벤트에서)
                                    ...
                                    # 전체 라운드는 두 플랫폼의 최대값을 사용
                                    if round_num > state.current_round:
                                        state.current_round = round_num
                                    # 전체 시간은 두 플랫폼의 최대값을 사용
                            ...
                            # 그래프 메모리 업데이트가 활성화되어 있으면, 활동을 Zep으로 전송
                            if graph_updater:
    ...
    def _check_all_platforms_completed(cls, state: SimulationRunState) -> bool:
        ...
        해당 actions.jsonl 파일이 존재하는지 확인하여 플랫폼이 활성화되었는지 판단
        ...
        # 최소 하나의 플랫폼이 활성화되어 있고 완료됨
        return twitter_enabled or reddit_enabled
    
    @classmethod
    def _terminate_process(cls, process: subprocess.Popen, simulation_id: str, timeout: int = 10):
        """
        크로스 플랫폼으로 프로세스와 하위 프로세스를 종료
        
        Args:
            process: 종료할 프로세스
            simulation_id: 시뮬레이션 ID(로그용)
            timeout: 프로세스 종료를 기다리는 제한 시간(초)
        """
        if IS_WINDOWS:
            ...
            # Unix: 프로세스 그룹으로 종료
            # start_new_session=True를 사용했으므로 프로세스 그룹 ID는 주 프로세스 PID와 동일
            ...
            try:
                ...
                # 제한 시간을 넘겨도 종료되지 않으면 SIGKILL을 강제로 전송
    ...
    def stop_simulation(cls, simulation_id: str) -> SimulationRunState:
        ...
        if state.runner_status not in [RunnerStatus.RUNNING, RunnerStatus.PAUSED]:
            raise ValueError(f"시뮬레이션이 실행 중이 아님: {simulation_id}, status={state.runner_status}")
        ...
        logger.info(f"시뮬레이션이 중지됨: {simulation_id}")
        return state
        ...
        with open(file_path, 'r', encoding='utf-8') as f:
            for line in f:
                ...
                try:
                    ...
                    # 동작 기록이 아닌 항목은 건너뜀(예: simulation_start, round_start, round_end 등의 이벤트)
                    if "event_type" in data:
                        continue
                    
                    # agent_id가 없는 기록은 건너뜀(Agent 동작 아님)
    ...
    def get_all_actions(
        ...
        """
        모든 플랫폼의 전체 동작 기록을 가져옵니다(페이지네이션 제한 없음)
        ...
        # Reddit 동작 파일 읽기(파일 경로에 따라 platform을 reddit로 자동 설정)
        reddit_actions_log = os.path.join(sim_dir, "reddit", "actions.jsonl")
        ...
        # 플랫폼별 파일이 없으면, 이전의 단일 파일 형식을 읽어봅니다
        if not actions:
    ...
    def cleanup_simulation_logs(cls, simulation_id: str) -> Dict[str, Any]:
        """
        시뮬레이션의 실행 로그를 정리합니다(시뮬레이션을 강제로 다시 시작하는 데 사용)
        ...
        - twitter_simulation.db(시뮬레이션 DB)
        - reddit_simulation.db(시뮬레이션 DB)
        - env_status.json(환경 상태)
        
        주의: 설정 파일(simulation_config.json)과 profile 파일은 삭제하지 않습니다
        ...
        files_to_delete = [
            ...
            "stdout.log",
            "stderr.log",
            "twitter_simulation.db",  # Twitter 플랫폼 DB
            "reddit_simulation.db",   # Reddit 플랫폼 DB
            "env_status.json",        # 환경 상태 파일
        ]
        
        # 삭제할 디렉터리 목록(액션 로그 포함)
    ...
    def cleanup_all_simulations(cls):
        ...
        서버 종료 시 호출하여 모든 하위 프로세스가 종료되도록 보장
        """
        # 중복 정리 방지
        ...
        # 정리할 내용이 있는지 확인(빈 프로세스의 프로세스 출력이 쓸모없는 로그를 찍는 것 방지)
        ...
        logger.info("모든 시뮬레이션 프로세스를 정리하는 중...")
        
        # 먼저 모든 그래프 메모리 업데이터를 중지(stop_all 내부에서 로그 출력)
        try:
        ...
        for simulation_id, process in processes:
            try:
                if process.poll() is None:  # 프로세스가 아직 실행 중
                    ...
                    # 동시에 state.json도 업데이트하여 상태를 stopped로 설정
                    try:
                        sim_dir = os.path.join(cls.RUN_STATE_DIR, simulation_id)
                        ...
                        if os.path.exists(state_file):
                            ...
                            logger.info(f"state.json 상태를 stopped로 업데이트함: {simulation_id}")
                        else:
    ...
    def register_cleanup(cls):
        ...
        # Flask debug 모드에서는 reloader 하위 프로세스에서만 정리 등록(실제로 앱을 실행하는 프로세스)
        # WERKZEUG_RUN_MAIN=true는 reloader 하위 프로세스임을 의미
        # debug 모드가 아니면 이 환경 변수가 없으므로 등록이 필요함
        is_reloader_process = os.environ.get('WERKZEUG_RUN_MAIN') == 'true'
        is_debug_mode = os.environ.get('FLASK_DEBUG') == '1' or os.environ.get('WERKZEUG_RUN_MAIN') is not None
        
        # debug 모드에서는 reloader 하위 프로세스에서만 등록; 비 debug 모드에서는 항상 등록
        if is_debug_mode and not is_reloader_process:
            _cleanup_registered = True  # 등록됨으로 표시하여 하위 프로세스가 다시 시도하는 것을 방지
            return
        
        # 기존 신호 처리기 저장
        original_sigint = signal.getsignal(signal.SIGINT)
        original_sigterm = signal.getsignal(signal.SIGTERM)
        # SIGHUP은 Unix 시스템(macOS/Linux)에서만 존재(Windows에는 없음)
        ...
        def cleanup_handler(signum=None, frame=None):
            """신호 처리기: 먼저 시뮬레이션 프로세스를 정리한 뒤 원래 처리기를 호출"""
            # 정리할 프로세스가 있을 때만 로그를 출력
            if cls._processes or cls._graph_memory_enabled:
                logger.info(f"신호 {signum} 수신, 정리 시작...")
            cls.cleanup_all_simulations()

            # 기존 신호 처리기를 호출하여 Flask가 정상 종료되도록 함
            if signum == signal.SIGINT and callable(original_sigint):
                original_sigint(signum, frame)
            elif signum == signal.SIGTERM and callable(original_sigterm):
                original_sigterm(signum, frame)
            elif has_sighup and signum == signal.SIGHUP:
                # SIGHUP: 터미널이 닫힐 때 전송됨
                ...
                # 원 처리기가 호출 불가능한 경우(예: SIG_DFL)에는 기본 동작을 사용
        ...
        try:
            ...
            # SIGHUP: 터미널 종료(Unix 시스템에서만)
            if has_sighup:
                signal.signal(signal.SIGHUP, cleanup_handler)
        except ValueError:
            # 메인 스레드가 아니어서 atexit만 사용 가능
    ...
    def check_env_alive(cls, simulation_id: str) -> bool:
        ...
        Returns:
            True는 환경이 살아있음을 의미하고, False는 환경이 이미 종료되었음을 의미합니다
        """
        sim_dir = os.path.join(cls.RUN_STATE_DIR, simulation_id)
        if not os.path.exists(sim_dir):
            return False
    ...
    def get_env_status_detail(cls, simulation_id: str) -> Dict[str, Any]:
        ...
        Returns:
            상태 상세 딕셔너리로, status, twitter_available, reddit_available, timestamp를 포함합니다
        """
        sim_dir = os.path.join(cls.RUN_STATE_DIR, simulation_id)
    ...
    def interview_agent(
        ...
        Args:
            ...
            platform: 지정 플랫폼(선택)
                ...
                - None: 양 플랫폼 시뮬레이션일 때 두 플랫폼을 동시에 인터뷰하고 통합 결과를 반환
            timeout: 제한 시간(초)
        ...
        if not ipc_client.check_env_alive():
            raise ValueError(f"시뮬레이션 환경이 실행 중이 아니거나 이미 종료되어 Interview를 수행할 수 없습니다: {simulation_id}")
    ...
    def interview_agents_batch(
        ...
        Args:
            ...
            platform: 기본 플랫폼(선택, 각 인터뷰 항목의 platform으로 덮어써짐)
                - "twitter": 기본으로 Twitter 플랫폼만 인터뷰
                - "reddit": 기본으로 Reddit 플랫폼만 인터뷰
                - None: 양 플랫폼 시뮬레이션일 때 각 Agent를 두 플랫폼에서 동시에 인터뷰
            timeout: 제한 시간(초)
    ...
    def interview_all_agents(
        ...
        동일한 질문으로 시뮬레이션 내의 모든 Agent를 인터뷰합니다
        ...
        Args:
            ...
            platform: 지정 플랫폼(선택)
                - "twitter": Twitter 플랫폼만 인터뷰
                - "reddit": Reddit 플랫폼만 인터뷰
                - None: 양 플랫폼 시뮬레이션 시 각 Agent가 두 플랫폼을 동시에 인터뷰
            timeout: 타임아웃 시간(초)
    ...
    def close_simulation_env(
        ...
        """
        시뮬레이션 환경 종료(시뮬레이션 프로세스를 중지하는 것이 아님)
        
        시뮬레이션에 환경 종료 명령을 보내 우아하게 명령 대기 모드에서 종료하게 합니다
        ...
        try:
            ...
            # 타임아웃은 환경이 종료 중이기 때문일 수 있습니다
            return {
                "success": True,
                "message": "환경 종료 명령이 전송되었습니다(응답 대기 타임아웃, 환경이 종료 중일 수 있음)"
            }
    ...
    def get_interview_history(
        ...
        """
        Interview 이력 기록 가져오기(데이터베이스에서 읽기)
        
        Args:
            simulation_id: 시뮬레이션 ID
            platform: 플랫폼 유형(reddit/twitter/None)
                - "reddit": Reddit 플랫폼의 이력만 가져오기
                - "twitter": Twitter 플랫폼의 이력만 가져오기
                - None: 두 플랫폼의 모든 이력 가져오기
            agent_id: 지정 Agent ID(선택, 해당 Agent의 이력만 가져오기)
            limit: 플랫폼별 반환 수 제한
